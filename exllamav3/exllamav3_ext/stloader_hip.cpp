...
{
    hipEvent_t events[STLOADER_SLOTS_PER_THREAD] = {};
    int event_device[STLOADER_SLOTS_PER_THREAD] = {};
    bool pending[STLOADER_SLOTS_PER_THREAD] = {};
    std::vector<std::pair<int, hipStream_t>> streams;
    int cur_slot = 0;
    int cur_device = -1;
    ...
    while (true)
    {
        ...
        while (!batch_failed.load(std::memory_order_acquire) && (run_i = batch_next_run.fetch_add(1)) < runs->size())
        {
            ...
            if (pending[cur_slot])
            {
                cr = hipEventSynchronize(events[cur_slot]);
                if (cr != hipSuccess) { batch_fail(std::string("stloader: event sync failed: ") + hipGetErrorString(cr)); break; }
            ...
            if (!stream)
            {
                cr = hipStreamCreateWithFlags(&stream, hipStreamNonBlocking);
                if (cr != hipSuccess) { batch_fail(std::string("stloader: stream create failed: ") + hipGetErrorString(cr)); break; }
            ...
            for (size_t ji = run.first_job; ji < run.first_job + run.num_jobs; ++ji)
            {
                ...
                if (cr == hipSuccess && job.bf16_to_fp16)
                    cr = inplace_bf16_to_fp16_cuda(reinterpret_cast<void*>(job.destination), job.bytesize / 2, stream);
                if (cr != hipSuccess)
                {
                    batch_fail(std::string("stloader: H2D copy failed: ") + hipGetErrorString(cr));
                    break;
                }
            }
            ...
            {
                hipEventDestroy(events[cur_slot]);
                events[cur_slot] = nullptr;
            }
            if (!events[cur_slot])
            {
                cr = hipEventCreateWithFlags(&events[cur_slot], hipEventDisableTiming);
                if (cr != hipSuccess) { batch_fail(std::string("stloader: event create failed: ") + hipGetErrorString(cr)); break; }
                event_device[cur_slot] = cur_device;
            }
            cr = hipEventRecord(events[cur_slot], stream);
            if (cr != hipSuccess)
            {
                int actual_dev = -2;
                hipGetDevice(&actual_dev);
                batch_fail(std::string("stloader: event record failed: ") + hipGetErrorString(cr) +
        ...
        for (auto& p : streams)
        {
            hipSetDevice(p.first);
            cr = hipStreamSynchronize(p.second);
            if (cr != hipSuccess)
                batch_fail(std::string("stloader: stream sync failed: ") + hipGetErrorString(cr));
        }
...
static std::string stloader_cuda_engine(std::vector<TensorLoadJob> const& jobs, size_t max_run_bytes)
{
    ...
    if (!pinned_pool)
    {
        ...
        {
            pinned_pool = nullptr;
            return std::string("stloader: failed to allocate pinned pool: ") + hipGetErrorString(cr);
        }
    }
    ...
    {
        hipSetDevice(dev);
        hipError_t cr = hipDeviceSynchronize();
        if (cr != hipSuccess)
        {
            hipSetDevice(restore_device);
            return std::string("stloader: device sync failed: ") + hipGetErrorString(cr);
        }
    }
    hipSetDevice(restore_device);
