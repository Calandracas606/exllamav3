...
#else
                // HIP's unified runtime API should handle all nodes, so this
                // branch is not expected to execute. Fail loudly if it does.
...
void Graph::inspect_graph()
{
    ...
    for (size_t i = 0; i < numNodes; ++i)
    {
        ...
        {
#if defined(USE_ROCM)
            // hipify doesn't map cudaKernelNodeParams → hipKernelNodeParams
