...
#if !defined(USE_ROCM)
// Category A + C: excluded from ROCm build (PyTorch native or PTX tensor-core)
#include "norm.cuh"
...
{
    // ---- Category B: always available (compile under both CUDA and ROCm) ----
...
#if !defined(USE_ROCM)
    // ---- Category A + C: only on CUDA (excluded from ROCm build) ----
