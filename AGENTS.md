...
> **POST-REFACTOR NOTE (current truth, updated for the v1.4.6 rebase):** The fork
> architecture changed fundamentally. The current lines are:
> - `staging` (= `origin/staging`, upstreamable PR line): rebased onto upstream/master
>   **v1.4.6** (499890c). Upstream's ENTIRE C++ extension builds on HIP and binds,
>   including v1.4.5/1.4.6 additions (PLE, n-gram, KDA, grouped norm). The Triton EXL3
>   linear line is DELETED. Platform seams: the lock-cascade inner
>   (`exl3_gemm_inner_rocm.cuh`), compat headers, bindings HIP branch. The fused MoE
>   and mgemm paths are ENABLED (capability-routed). 13 commits over upstream/master.
